 ...
 * 公開操作は本アプリで唯一 @Roles("ADMIN", "EDITOR") が課された変更操作（設計書⑦ RBAC）であるため、
 * Phase6「監査ログ」の最初の記録対象とする（actorIdはコントローラでJWT検証済みのrequest.user.userIdを渡す）。
 */
@Injectable()
export class PublishRegulationVersionUsecase {
  ...
  async execute(versionId: string, actorId: string): Promise<PublishVersionResult> {
    ...
    await this.recordAuditLogUsecase.execute({
      ...
      target: `regulation_version:${result.versionId}`,
    });
