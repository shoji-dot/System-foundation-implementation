...
| `STRIPE_SECRET_KEY`                              | Stripe Checkout/Portal/Webhook用シークレットキー（Phase7 7-1、infrastructure/external/billing）          | Stripeダッシュボード(テストモード)のキー        | Railway Variablesに設定（Secretsとして扱う、コミット・ログ出力禁止） |
| `STRIPE_WEBHOOK_SECRET`                          | Webhookエンドポイントの署名シークレット(`whsec_...`、Stripeダッシュボード「Webhooks」で発行)             | Stripe CLI/ダッシュボードのテスト用シークレット | Railway Variablesに設定（Secretsとして扱う、コミット・ログ出力禁止） |
| `STRIPE_PRICE_PRO_*` / `STRIPE_PRICE_BUSINESS_*` | プラン×月額/年額のStripe Price ID（4種、設計変更書⑥）                                                    | Stripeダッシュボードで発行したPrice ID          | 同左（本番用Price IDを個別設定）                                     |
| `WEB_APP_URL`                                    | Checkout/Portalのsuccess_url・cancel_url・return_url組み立てに使用するフロントエンドのベースURL          | `http://localhost:3000`                         | VercelのProductionドメイン                                           |
