...
LifecycleTemplate     // id, jurisdictionId, deviceCategory(enum: CLASS_I..IV, SAMD, IVD, ACTIVE), procedureType(届出/認証/承認/510k/CE…), status(draft/published), version
...
```
GET  /lifecycle/templates?jurisdiction=&deviceCategory=&procedureType=   # マスタ一覧(Free可)
